...
function cacheLink()
    ...
    if lootDecoded.type ~= "created" or not lootDecoded.message or not lootDecoded.message.loot_url then
        onMessage("LootLabs ошибка: " .. tostring(lootDecoded.message));
        return false, "lootlabs error";
    end

    cachedLink = lootDecoded.message.loot_url;   -- https://loot-link.com/s?xxxx
    cachedTime = fOsTime();
