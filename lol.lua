...
local LOOTLABS_BASE_URL = "https://loot-link.com/s?xwgrpFTc";
...
function cacheLink()
    ...
    if not okDecode or type(decoded) ~= "table" or not decoded.success then
        onMessage("Сервер вернул не JSON или ошибку.");
    ...
    local encryptResponse, encryptErr = safeRequest({
        ...
        Method = "POST",
        Body = lEncode({
            destination_url = platorelayUrl,
            api_token = LOOTLABS_API_KEY
        }),
        Headers = { ["Content-Type"] = "application/json" }
    });

    if not encryptResponse then
        local msg = "LootLabs недоступен: " .. tostring(encryptErr);
        onMessage(msg);
        return false, msg;
    end
    ...
    local encryptedData = encDecoded.message;

    -- ШАГ 3: собираем финальную ссылку
    cachedLink = LOOTLABS_BASE_URL .. "&data=" .. encryptedData;
    cachedTime = fOsTime();
