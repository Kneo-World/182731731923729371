...
-------------------------------------------------------------------------------
--! platoboost library (ВСЁ ЧЕРЕЗ api.platorelay.com)
-------------------------------------------------------------------------------
...
local onMessage = function(message)
    ...
    print("[Platorelay] " .. tostring(message))
end
...
--! ========== ЕДИНЫЙ ХОСТ ДЛЯ ВСЕГО API ==========
local host = "https://api.platorelay.com"

--! кэш ссылки на кейсистему
...
function cacheLink()
    ...
    if not response then
        local msg = "Нет связи: " .. tostring(err);
        onMessage(msg);
        return false, msg;
    end
    ...
    if response.StatusCode == 200 then
        ...
        if not okDecode or type(decoded) ~= "table" then
            onMessage("Сервер вернул не JSON (код " .. tostring(response.StatusCode) .. ")");
    ...
    elseif response.StatusCode == 429 then
        onMessage("rate limit, wait 20 sec.");
        return false, "rate limit";
    end

    local msg = "Ошибка сервера: " .. tostring(response.StatusCode);
    onMessage(msg);
    return false, msg;
end
...
local verifyKey = function(key)
    if requestSending == true then
        onMessage("wait, request sending.");
        return false;
    end
    requestSending = true;
    ...
    elseif response.StatusCode == 429 then
        ...
        onMessage("status: " .. tostring(response.StatusCode));
        return false;
    end
end
...
getBtn.MouseButton1Click:Connect(function()
    ...
    if ok then
        setStatus("Ссылка в буфере!", Color3.fromRGB(100, 220, 130))
    else
