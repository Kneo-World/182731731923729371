...
-------------------------------------------------------------------------------
--! platoboost library (PLATORELAY — актуальные домены)
-------------------------------------------------------------------------------
...
local function safeRequest(opts)
    ...
    if type(res) ~= "table" then return nil, "пустой ответ" end
    return res, nil
end

--! ============ PLATORELAY ДОМЕНЫ ============
local host     = "https://api.platorelay.com"   -- API-запросы
local authHost = "https://auth.platorelay.com"  -- кейсистема
...
local verifyKey = function(key)
    ...
    if response.StatusCode == 200 then
        ...
        if not okDecode then
            onMessage("Сервер вернул не JSON. Код: 200, но не JSON.");
            return false;
        end
...
verifyBtn.MouseButton1Click:Connect(function()
    ...
    task.spawn(function()
        ...
        if ok and result == true then
            ...
            if not okLoad then
                warn("[Kneo] Ошибка: " .. tostring(loadErr))
                pcall(function()
                    game:GetService("StarterGui"):SetCore("SendNotification", {
